...
u = [0]*(m+1);  u0 = [0]*(m+1); uf = [0]*(m+1);  dt = beta*dx/c;
T_final = 0.5; n = int(T_final/dt)             # N time steps

graph1 = graph(width=600, height=500, xtitle = 'x', xmin=0,
	xmax=1,ymin=0, ymax=1, ytitle = 
	'u(x), Cyan=exact, Green=Numerical', title='Advect Eqn:' 
	'Initial (red), Exact (cyan),Numerical (green)')
initfn = gcurve(color = color.red);
exactfn = gcurve(color = color.cyan)
numfn = gcurve(color = color.green)       # Numerical solution
