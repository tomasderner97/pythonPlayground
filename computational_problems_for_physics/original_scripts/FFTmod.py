...
import numpy as np   # from sys import version 
max = 2100;  points = 1026; N = 16              # Power of 2
isign = -1                              # -1, 1 TF, inverse TF          
data = np.zeros((max));  dtr  = np.zeros((points,2))
...
def FFT(N, isign):                        # FFT of dtr[n,2]
    ...
    for i in range(1,n+2, 2):
        if (i-j) < 0 :   
            ...
            data[i] = tempr
            data[i+1] = tempi 
        m = n//2;   # NB divide to remain integer
        while (m-2 > 0): 
            if  (j-m) <= 0 :  break
            j = j-m
            m = m//2 # integer part
...
print('\n    Fourier Transform \n  i      Re      Im    ')
for i in range(0,N): print(" %2d %9.5f %9.5f " %(i,dtr[i,0],dtr[i,1]) )
