...
while end < 5:
    rate(150)       
    Exx[1:xmax-1,1] = Exx[1:xmax-1,0] + beta*(Hyy[0:xmax-2,0] 
    	    - Hyy[2:xmax,0])
    Eyy[1:xmax-1,1] = Eyy[1:xmax-1,0] + beta*(Hxx[0:xmax-2,0] 
    	    - Hxx[2:xmax,0])
    Hyy[1:xmax-1,1] = Hyy[1:xmax-1,0] + beta*(Exx[0:xmax-2,0] 
    	    - Exx[2:xmax,0])
    Hxx[1:xmax-1,1] = Hxx[1:xmax-1,0] + beta*(Eyy[0:xmax-2,0] 
    	    - Eyy[2:xmax,0])
    Ex[1:xmax-1,1] =  Ex[1:xmax-1,0]  + beta*(Hy[0:xmax-2,0]  
    	    - Hy[2:xmax,0])   
    Ey[1:xmax-1,1] =  Ey[1:xmax-1,0]  + beta*(Hxx[0:xmax-2,0] 
    	    - Hxx[2:xmax,0])
    Hy[1:xmax-1,1] =  Hy[1:xmax-1,0]  + beta*(Ex[0:xmax-2,0]  
    	    - Ex[2:xmax,0])       
    Hx[1:xmax-1,1] =  Hx[1:xmax-1,0]  + beta*(Eyy[0:xmax-2,0] 
    	    - Eyy[2:xmax,0])
    ...
    Exx[0,1] = Exx[0,0] + beta*(Hyy[xmax-2,0] - Hyy[1,0]) # Periodic
    Eyy[0,1]=  Eyy[0,0] + beta*(Hxx[xmax-2,0] - Hxx[1,0]) 
    Hyy[0,1] = Hyy[0,0] + beta*(Exx[xmax-2,0] - Exx[1,0]) # Periodic
    Hxx[0,1] = Hxx[0,0] + beta*(Eyy[xmax-2,0] - Eyy[1,0]) 
    Hyy[xmax-1,1] = Hyy[xmax-1,0] + beta*(Exx[xmax-2,0] 
    	    - Exx[1,0])  
    Hxx[xmax-1,1] = Hxx[xmax-1,0] + beta*(Eyy[xmax-2,0] 
    	    - Eyy[1,0]) 
    Exx[xmax-1,1] = Exx[xmax-1,0] + beta*(Hyy[xmax-2,0] 
    	    - Hyy[1,0])  
    Eyy[xmax-1,1] = Eyy[xmax-1,0] + beta*(Hxx[xmax-2,0] 
    	    - Hxx[1,0])
    Ex[0,1] = Exx[0,0] + beta*(Hyy[xmax-2,0] - Hyy[1,0]) # Periodic
    Ey[0,1] = Eyy[0,0] + beta*(Hxx[xmax-2,0] - Hxx[1,0]) 
    Hy[0,1] = Hyy[0,0] + beta*(Exx[xmax-2,0] - Exx[1,0]) # Periodic
    Hx[0,1] = Hxx[0,0] + beta*(Eyy[xmax-2,0] - Eyy[1,0])    
    Hy[xmax-1,1] = Hy[xmax-1,0] + beta*(Ex[xmax-2,0]
    	    - Ex[xmax-100,0])
    Hx[xmax-1,1] = Hx[xmax-1,0] + beta*(Ey[xmax-2,0] 
    	    - Ey[1,0]) 
    Ex[xmax-1,1] = Ex[xmax-1,0] + beta*(Hy[xmax-2,0] 
    	    - Hy[xmax-100,0])
    Ey[xmax-1,1] = Ey[xmax-1,0] + beta*(Hxx[xmax-2,0] 
    	    - Hxx[1,0])
    PlotFields(ti)                      
    k = arange(101,202)
    Ex[101:202,1] = 0.1*cos(-2*pi*k/100-0.005*pi*(k-101)
    	    + 2*pi*j/4996.004)
    Hy[101:202,1] = 0.1*cos(-2*pi*k/100-0.005*pi*(k-101)
    	    + 2*pi*j/4996.004)
