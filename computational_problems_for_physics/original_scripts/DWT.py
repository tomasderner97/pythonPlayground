...
from vpython import *;  import numpy as np
...
def daube4(f, n, sign):      # DWT if sign >= 0, inverse if < 0
    ...
    tr = np.zeros( (n + 1), float)                  # Temporary  
    if n < 4 : return
    mp = n//2                                           
    mp1 = mp + 1                                # midpoint + 1
    if sign >= 0:                                     # DWT
        ...
        if n > 128:                                  # Scale 
            ...
            speed = 8                               # Lower rate
        if transfgr1:
            transfgr1.visible = False
            transfgr2.visible = False
            del transfgr1
            del transfgr2
        transfgr1 = graph(x=0, y=0, width=600, height=400,
		    title='Wavelet TF, down sample + low pass',
		    xmax=maxx, xmin=0, ymax=maxy, ymin=miny)
        transf  = gvbars(delta=2.*n/N,color=color.cyan)     
        transfgr2 = graph(x=0, y=400, width=600, height=400,
...
f = np.zeros( (N + 1), float)                  # data vector
