...
from vpython import *; import numpy as np; import random
...
Natom = 25; Nmax =  25; Tinit = 2.0; dens = 1.0    # Density (1.20 for fcc)
t1 = 0
x  = np.zeros( (Nmax), float);  y  = np.zeros( (Nmax), float)                                 
vx = np.zeros( (Nmax), float);  vy = np.zeros( (Nmax), float)                 
fx = np.zeros( (Nmax, 2), float);  fy = np.zeros( (Nmax, 2), float)                       
...
def initialposvel():                            # Initialize
    ...
    for j in range(0,Natom):
        xc = 2*x[j] - 4
        yc = 2*y[j] - 4
        atoms.append(sphere(pos=vec(xc,yc,0), radius=0.5,
        	color=color.red))
...
def Forces(t, w, PE, PEorW):                            # Forces
    ...
    PE = 0.                   
    for i in range(0, Natom): fx[i][t] = fy[i][t]  = 0.0   
    for i in range( 0, Natom-1 ):
          for j in range(i + 1, Natom):
              dx = x[i] - x[j]                         
              dy = y[i] - y[j]                         
              if (abs(dx) > 0.50*L): dx = dx - sign(L, dx) # Closest image
              if (abs(dy) > 0.50*L): dy = dy - sign(L, dy)
              r2 = dx*dx  +  dy*dy                         
              if (r2 < r2cut):                      
                  if (r2 ==  0.):  r2 = 0.0001 # To avoid 0 denominator 
                  invr2 = 1./r2                      
...
def timevolution():
    avT = 0.0; avP = 0.0;   Pavg = 0.0;    avKE = 0.0;  avPE = 0.0
    t1 = 0;  PE = 0.0; h = 0.031                         
    hover2 = h/2.0 
    KE = 0.0
    w = 0.0
    ...
    while 1:
        ...
        avT = avT + T;  avP = avP + P                                           
        avKE = avKE + KE;   avPE = avPE + PE                                
        time += 1
        t = time
        if (t==0): t=1
        Pavg  = avP /t;   eKavg = avKE /t;   ePavg = avPE /t; Tavg  = avT /t
        pre = (int)(Pavg*1000)
        Pavg = pre/1000.0
