...
originalsignal=graph(x=0, y=0, width=600, height=200, \
        title='Input Signal',xmin=0,xmax=12,ymin=-20,ymax=20)
orsigraph = gcurve(color=color.red)
invtrgr = graph(x=0, y=200, width=600, height=200,
         title='Inverted Transform',xmin=0,xmax=12,ymin=-20,ymax=20)
invtr = gcurve(color=color.green)
...
dTau =  W/noTau;    dS =  (W/iS)**(1./noS); # Need tiny s for hi freq
maxY =  0.001;      sig =  np.zeros((noPtsSig), float)     # Signal
...
print("Working at finding transform, count 20")
for i in range( 0, noS):
    s *= dS                                        # Scaling
    tau = iT
    print(i)
...
for i in range( 0, noS):
    for j in range( 0, noTau):
        if Yn[i,j] > maxY or Yn[i,j] < - 1*maxY: maxY = abs(Yn[i,j])                     
tau =  iT
s =  iS
print("normalize")      
for i in range( 0, noS):
